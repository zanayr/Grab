...
export class GrabElement {
    ...
    constructor(htmlElement) {
        ...
        Object.defineProperties(this, {
            ...
            __node: {
                // The owned html DOM node (owned html node)
                enumerable: false,
                value: node,
            },
            attributeList: {
 ...
 * @param {string} path is a url
 * @param {Object} options is an object
 * @returns a Promise
 */
export function get(path, options={}) {
    fetch(path, Object.assign({}, options, {method: 'GET'}));
}
/**
...
export function post(path, options={}) {
    fetch(path, Object.assign({}, options, {method: 'POST'}));
}
/**
 * Create a POST request to an api endpoint with a form's content
 * @param {GrabElement} form is a `GrabElement` of a form element
 * @param {string} path is a url
 * @returns a Promise
 */
export function send(form, path) {
    const data = new URLSearchParams(new FormData(form.node));
    return fetch(path, {body: data, method: 'POST'});
}
/**
 * Create an event to be fired when the window loads and is passed the event object
 * @param {Function} callback is a function to be called when the page loads
 * @returns undefined
 */
